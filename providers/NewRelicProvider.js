...
class NewRelicProvider extends ServiceProvider {
  ...
  boot() {
    ...
    const newRelicPackage = require(path.join(appRoot, 'node_modules/newrelic/package.json'));
    const _this = this;
    ...
    newrelic.instrumentWebframework(
      ...
      function(shim, Server) {
        ...
        shim.wrap(Server.prototype, ['registerGlobal', 'registerNamed', 'use'], function(
          shim,
          originalFn
        ) {
  ...
  wrapRoute(shim, route, handler) {
    ...
    if (shim.isFunction(handler)) {
      ...
      const controllerInstance = resolver.forDir('httpControllers').resolve(controllerName);
      const fn = controllerInstance[action];
      controllerInstance[action] = this.wrapControllerMethod(shim, controllerName, action, fn);
      const controllerNamespace = resolver.forDir('httpControllers').translate(controllerName);
  ...
  wrapResource(shim, resource, controllerName) {
    ...
    wrappableMethods.forEach(method => {
      ...
      controllerInstance[method] = this.wrapControllerMethod(shim, controllerName, method, fn);
    });
  ...
  wrapMiddleware(shim, middleware) {
    ...
    middleware.forEach(namespace => {
      ...
      if (binding) {
        ...
        middleware.handle = this.wrapMiddlewareHandler(shim, namespace, handleFn);
      ...
      Middleware.prototype.handle = this.wrapMiddlewareHandler(shim, namespace, handleFn);
